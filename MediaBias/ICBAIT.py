...
def train_save_model(dataset,embd):
    ...
    concat_input = Dense(768, activation='relu')(concat_input)
    concat_input= Dropout(0.5)(concat_input)
    concat_input = Dense(256, activation='relu')(concat_input)
    concat_input= Dropout(0.5)(concat_input)
    ...
    if os.path.isfile('models/'+ embd + '/' + dataset + '/MODEL.h5') == False:
        ...
        gate_model.fit([trainque_feature,trainsum_feature], trainans, epochs=10, batch_size=96, validation_data=([validque_feature,validsum_feature], validans), callbacks=callbacks_list, verbose=1)
