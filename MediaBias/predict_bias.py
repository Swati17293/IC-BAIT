...
def train_save_model(dataset,embd):
    ...
    encode_title_ = Dense(768, activation='relu')(encode_title_)
    encode_title_= Dropout(0.5)(encode_title_)
    encode_title_ = Dense(256, activation='relu')(encode_title_)
    encode_title_= Dropout(0.5)(encode_title_) 

    gate_model = Dense(3, activation='softmax')(encode_title_)
    ...
    if os.path.isfile('models/'+ embd + '/' + dataset + '/MODEL.h5') == False:
        ...
        gate_model.fit([trainque_feature], trainans, epochs=10, batch_size=96, validation_data=([validque_feature], validans), callbacks=callbacks_list, verbose=1)
