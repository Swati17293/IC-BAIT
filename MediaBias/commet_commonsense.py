...
for lines in f1:
    processed_headlines.append(lines.split(',')[2].replace('\n',''))
...
for lines in f:
    ...
    strencode = headline.encode("ascii", "ignore")
    headline = strencode.decode()

    headline_pro = processed_headlines[line_cnt].replace('\n','')
    strencode = headline_pro.encode("ascii", "ignore")
    headline_pro = strencode.decode()
